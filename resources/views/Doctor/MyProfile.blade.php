...
@section('section')
    ...
    <main class="flex-1 p-6">
        <div class="bg-white p-6 rounded shadow">
            ...
            <p class="text-gray-500 text-sm">Last updated on: {{ date("M d,Y h:ia", strtotime($fetchRecord->updated_at)) }}</p>


            <form action="{{ route('Doctor.UpdateProfile') }}" method="POST" enctype="multipart/form-data" autocomplete="off">
                @csrf
                <h4 class="text-lg font-semibold my-4">Personal Information</h4>
                <img src="{{ asset('Doctors/Profile/' . $fetchRecord->profilePicture) }}" alt="my-profile"
                    class="size-32 my-5 object-cover rounded-full">
                <div class="w-full grid grid-cols-1 md:grid-cols-3 gap-5 space-y-1 md:space-y-0">
                    ...
                    <div class="flex flex-col">
                        ...
                        <input type="email" name="emailAddress" id="emailAddress" value="{{ $fetchRecord->emailAddress }}"
                            readonly class="bg-white px-3 py-1 rounded-md border border-slate-300 focus:outline-none">
                    </div>
                    ...
                    <div class="flex flex-col">
                        ...
                        <select name="department" id="department"
                            ...
                            @foreach ($departmentArr as $value)
                                <option value="{{ $value }}"
                                    {{ $fetchRecord->department == $value ? 'selected' : '' }}>{{ $value }}
                                </option>
                            @endforeach
                        </select>
                    </div>
                    ...
                    <div class="flex flex-col">
                        ...
                        <select name="yearsOfExperience" id="yearsOfExperience"
                            ...
                            @for ($i = 1; $i <= 10; $i++)
                                <option value="{{ $i }}"
                                    {{ $fetchRecord->yearsOfExperience == $i ? 'selected' : '' }}>{{ $i }}
                                </option>
                            @endfor
                        </select>
                    </div>
                    ...
                    <div class="flex flex-col">
                        ...
                        <input type="text" name="licenseNumber" id="licenseNumber"
                            value="{{ $fetchRecord->licenseNumber }}"
                            class="bg-white px-3 py-1 rounded-md border border-slate-300 focus:outline-none">
                    </div>
                    ...
                    <div class="flex flex-col">
                        ...
                        <input type="number" name="consultationFee" id="consultationFee"
                            value="{{ $fetchRecord->consultationFee }}"
                            class="bg-white px-3 py-1 rounded-md border border-slate-300 focus:outline-none">
                    </div>
                    ...
                    <div class="flex flex-col">
                        ...
                        <select name="status" id="status"
                            class="bg-white px-3 py-1 rounded-md border border-slate-300 focus:outline-none capitalize">
                            <option value="Available" {{ $fetchRecord->status == 'Available' ? 'selected' : '' }}>Available
                            </option>
                            <option value="Unavailable" {{ $fetchRecord->status == 'Unavailable' ? 'selected' : '' }}>
                                Unavailable</option>
                        </select>
                    </div>
                    ...
                    <div class="flex flex-col">
                        ...
                        <input type="text" readonly value="{{ $myShift->applicableDays }}"
                            class="bg-white px-3 py-1 rounded-md border border-slate-300 focus:outline-none">
                    </div>
