...
@section('main-section')
    <div class="w-full flex flex-col md:flex-row justify-between items-center">
            ...
            <form action="{{ route('VerifyUserCredentials') }}" class="my-10" autocomplete="off" method="post">
                ...
                <div class="flex flex-col">
                    ...
                    <input type="email" name="email" id="email"
                        class="border border-[#9CA3AF] rounded-md p-2 focus:outline-none" value="{{old("email")}}">
                    @error('email')
                        <span class="text-red-700 text-sm">{{ 'Please enter your registered email' }}</span>
                    @enderror
                </div>
                ...
                <div class="mt-5 flex flex-col">
                    ...
                    @error('password')
                        <span class="text-red-700 text-sm">{{ 'Please enter your password' }}</span>
                    @enderror
                </div>
        ...
        <div
            class="w-80 hidden md:block md:w-3/6 bg-[url('images/authentication.png')] min-h-screen bg-cover bg-no-repeat mb-0">
        </div>
    </div>
@endsection
