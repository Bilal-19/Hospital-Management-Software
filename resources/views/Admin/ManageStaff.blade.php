                    ...
                    @foreach ($users as $record)
                        <tr class="border-b border-gray-300">
                            ...
                            <td class="p-3 flex space-x-2">
                                <a href="{{ route('ResetPassword', ['id' => $record->id]) }}"
                                    class="bg-blue-700 text-white px-2 py-1 rounded-md">
                                    <i class="fa-solid fa-key "></i> Reset Password
                                </a>
                                <a href="{{ route('DeleteAccount', ['id' => $record->id]) }}"
                                    class="bg-red-700 text-white px-2 py-1 rounded-md">
                                    <i class="fa-solid fa-trash"></i> Delete
                                </a>
                                @if ($record->role != 'Patient')
                                    <a href="{{ route('Admin.GeneratePaySlip', ['id' => $record->id]) }}"
                                        class="bg-green-700 text-white px-2 py-1 rounded-md"><i
                                            class="fa-solid fa-wallet"></i> Generate Pay Slip</a>
                                @endif
                            </td>
                        </tr>
                    @endforeach
